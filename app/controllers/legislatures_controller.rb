class LegislaturesController < ApplicationController
  ...
  def create
    ...
    if @legislature.save
      redirect_to representatives_url, notice: 'Legislatura creada exitosamente.'
    else
  ...
  def update
    ...
    if @legislature.update_attributes params[:legislature]
      redirect_to representatives_url, notice: 'Legislatura editada exitosamente.'
    else
  ...
  def destroy
    @legislature = Legislature.new params[:id]    
    @legislature.destroy
    redirect_to representative_url
  end
end
